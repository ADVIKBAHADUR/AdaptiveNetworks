...
def main(nodes, q, iters, d):
    ...
    Di = d
    K = 0.2
    NODES = random.randint(nodes - 10, nodes + 10)
    AR = 0.5
    BR = AR
    CR = 9
    Q = q
    ...
    global t, synce
    t, synce = 0, 0.0
    ratio = random.randint(10, 50)/100
    ...
    for i in (Nodes_list_B):
        G.nodes[i]['Adaptive'] = is_adaptive(ratio)
        G.nodes[i]['x_state'] = 0
        G.nodes[i]['y_state'] = 0 
        G.nodes[i]['z_state'] = 0
    ...
    def Synchronisation(graph):
        ...
        for m in range(iters):
            ...
            for i in range(NODES):
                iter_thru = np.linspace(t, tplus, 6)
                for l in iter_thru:
                    ...
                    #print(z_dot)
                    tplus = l
                    xplus = graph.nodes[i]['x_state'] + 0.1*(x_dot)
                    yplus = graph.nodes[i]['y_state'] + 0.1*(y_dot)
                    zplus = graph.nodes[i]['z_state'] + 0.1*(z_dot)   
